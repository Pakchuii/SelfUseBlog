            ...
            ${(article.comments || []).map(c => `
              <div style="background: rgba(0,0,0,0.02); padding: 1rem; border-radius: 8px;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 0.5rem;">
                  ...
                  <span style="font-size: 0.8rem; color: #888;">${(c.date && c.date !== 'null') ? new Date(c.date).toLocaleString() : '未知时间'}</span>
                </div>
...
export function renderArticlesList(container: HTMLElement, onNavigate: (to: string, params?: any) => void) {
...
export function renderGallery(container: HTMLElement) {
  ...
  fetch('http://localhost:3001/api/assets')
    ...
    .then(urls => {
      ...
      // Filter out URLs that no longer exist in assets and non-images
      galleryUrls = galleryUrls.filter(u => urls.includes(u) && !u.match(/\.(mp4|webm)$/i));
      
      // Add new images that aren't in the custom order yet
      const newImages = urls.filter((u: string) => !u.match(/\.(mp4|webm)$/i) && !galleryUrls.includes(u));
      const finalImages = [...galleryUrls, ...newImages];
